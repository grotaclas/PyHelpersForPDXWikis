...
[[Category:Events]]"""
    ...
    def generate_formable_countries_cargo(self):
        return self.surround_with_autogenerated_section('formable_countries', self.get_formable_countries_cargo(), add_version_header=True)
    
    def get_formable_countries_cargo(self):
        formable_countries = self.parser.formable_countries.values()
        formable_country_table_data = []

        for formable_country in formable_countries:
            formable_country_table_data.append({
                'name': formable_country.name,
                'display_name': formable_country.display_name,
                ...
                'continents': ';'.join([continents for continents in formable_country.continents]),  # continents: list[str]
                'country_name': formable_country.country_name,  # country_name: <class 'str'>
                'form_effect': self.formatter.format_effect(formable_country.form_effect),  # form_effect: <class 'eu5.effect.Effect'>
                'level': formable_country.level,  # level: <class 'int'>
                'locations': ';'.join([locations.display_name if locations else '' for locations in formable_country.locations]),  # locations: list[eu5.eu5lib.Location]
                'potential': self.formatter.format_trigger(formable_country.potential),  # potential: <class 'eu5.trigger.Trigger'>
                'provinces': ';'.join([provinces.display_name if provinces else '' for provinces in formable_country.provinces]),  # provinces: list[eu5.eu5lib.Province]
                'regions': ';'.join([regions.display_name if regions else '' for regions in formable_country.regions]),  # regions: list[eu5.eu5lib.Region]
