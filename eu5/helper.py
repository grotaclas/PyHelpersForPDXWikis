...
class Eu5OneTypeHelper(OneTypeHelper):
    ...
    def __init__(self, folder, depth=0, ignored_toplevel_keys: list = None, ignored_keys: list = None, class_name_map=None, ignore_based_on_data=None, workarounds: list[ParsingWorkaround] = None):
    ...
    def get_data(self):
        ...
        return self.parser.parser.parse_files(glob, workarounds=self.workarounds)
    ...
    def _update_keys_from_data(self, data, keys, ignored_keys: list):
        super()._update_keys_from_data(data, keys, ignored_keys)
    ...
    def print_possible_table_columns_with_autogenerated_section_tags(self, parser_property_name: str, var_name: str, ignored_attributes: Set = None,
        ...
        print(f'{function_dec_indent_str}def {self.generate_function_name(parser_property_name, cargo)}(self):')
        print(
            f'{base_indent_str}return self.surround_with_autogenerated_section(\'{parser_property_name}\', self.{self.table_generator_function_name(parser_property_name, cargo)}(), add_version_header=True)')
        self.print_possible_table_columns(parser_property_name, var_name, ignored_attributes, cargo, indent)

    def table_generator_function_name(self, parser_property_name: str, cargo: bool = False) -> str:
        if cargo:
            suffix = 'cargo'
        else:
            suffix = 'table'
        return f'get_{parser_property_name}_{suffix}'

    def generate_function_name(self, parser_property_name: str, cargo: bool = False) -> str:
        if cargo:
            suffix = 'cargo'
        else:
            suffix = 'table'
        return f'generate_{parser_property_name}_{suffix}'
    ...
    def print_possible_table_columns(self, parser_property_name: str, var_name: str, ignored_attributes: Set = None,
        ...
        if table_generator_function is None:
            table_generator_function = self.table_generator_function_name(parser_property_name, cargo)
        ...
        if cargo:
            print(f"""{base_indent_str}return self.create_cargo_template_calls('{self.get_possible_class_name()}', {result_var_name})""")
...
def advanced_parsing():
    Eu5OneTypeHelper(
        ...
        ignored_keys=list(Effect.get_all_effects()),
        ...
        workarounds=[ScriptedWorkaround()],
        # ignore_based_on_data=lambda key, data, depth:  'dynamic_historical_event' in data or (depth == 1 and key != 'dynamic_historical_event'),
        ignore_based_on_data=lambda key, data, depth: (depth == 1 and key != 'dynamic_historical_event') or ':' in key,

    ).print_examples_and_code()

def table_generators(table_options: list, cargo=False):
    ...
    for var_name, folder, parser_property_name, helper in options_helpers:
        print(f"    '{parser_property_name}': generator.{helper.table_generator_function_name(parser_property_name, cargo)}(),")
...
def generate_tables_all():
    table_generators([
        ('in_game/common/heir_selections', 'heir_selection', 'heir_selections'),
