...
class FileGenerator:
    ...
    def create_wiki_list(elements: list[str], indent=1, no_list_with_one_element=False, prefix_with_linebreak=True, format_with_icon=False) -> str:
        return WikiTextFormatter.create_wiki_list(elements, indent, no_list_with_one_element=no_list_with_one_element, prefix_with_linebreak=prefix_with_linebreak, format_with_icon=format_with_icon)

    def surround_with_autogenerated_section(self, section_name, contents, generator=name_of_this_tool, add_version_header=False, custom_warning_message: str = None, skip_generator_comment=False, lua=False):
...
{warning_message}'''
        if lua:
            ...
            if generator_comment:
                generator_comment = f'<!--{generator_comment}-->'
        new_contents_with_sections = f'''<section begin={full_section_name}/>{generator_comment}{version_tag}{contents}<section end={full_section_name}/>'''
        if lua:
            new_contents_with_sections = '--[[\n    ' + new_contents_with_sections + '\n]]'
