...
class Helper:
    ...
    def find_all_keys_in_folder(self, folder, depth=0, ignored_toplevel_keys=None):
        keys = {}
        ...
        for filename, filedata in self.get_data(folder):
            for toplevelkey, data in filedata:
                if toplevelkey in ignored_toplevel_keys:
                    continue
                if depth == 0:
                    self._update_keys_from_data(data, keys)
                else:
                    for n2, d2 in data:
                        if depth == 1:
                            self._update_keys_from_data(d2, keys)
                        else:
                            for n3, d3 in d2:
                                if depth == 2:
                                    self._update_keys_from_data(d3, keys)
                                else:
                                    for n4, d4 in d3:
    ...
    def get_value_for_example(self, value):
        if isinstance(value, collections.abc.Hashable):
            ...
            return type(value)
    def _update_keys_from_data(self, data, keys):
        if isinstance(data, list):
            for item in data:
                self._update_keys_from_data(item, keys)
        else:
            for k, v in data:
                if k not in keys:
                    keys[k] = set()
