...
''']
    ...
    def generate_data_link_action_list(self):
        result = []
        for name, section in self.get_data_link_action_sections().items():
            result.append(f'=== {name} ===')
            result.append(self.surround_with_autogenerated_section(name, section))
        return result

    def get_data_link_action_sections(self) -> dict[str, str]:
        result = {}
        for action in self.parser.data_link_actions.values():
            result[action.name] = f'\n====Usage====\n{self.create_wiki_list(action.entities, format_with_icon=True)}\n====Effect====\n<section begin={action.name} />{self.create_wiki_list(action.card.get_effects())}<section end={action.name} />'
        return result
